...
import { CalendarDays, Dumbbell, Play, Ruler } from 'lucide-react'
...
export function HomePage() {
  ...
  const weekStats = useMemo(() => {
    ...
    const list = sessions.filter((s) => new Date(s.startedAt).getTime() >= cutoff)
    return { count: list.length, volume: list.reduce((acc, s) => acc + (s.totalVolumeKg ?? 0), 0) }
  }, [sessions])
    ...
    <div className="flex flex-col gap-4">
      ...
      {activeRoutine && todayDay && (
        <Card className="flex flex-col gap-3">
          <p className="text-xs text-gray-500 dark:text-gray-400">Hoy te toca</p>
          <div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <Card className="flex flex-col gap-1">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">Entrenamientos (7 días)</p>
        </Card>
        <Card className="flex flex-col gap-1">
          <p className="text-2xl font-bold">{weekStats.volume.toLocaleString()}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">Kg movidos (7 días)</p>
        </Card>
      </div>
      ...
      {latestWeight && (
        <Card className="flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400">Último peso</p>
