...
async function getPreviousSets(exerciseId: string) {
  ...
  for (const session of sorted) {
    const wex = wExercises.find((we) => we.workoutSessionId === session.id && we.exerciseId === exerciseId)
    if (!wex) continue
    const sets = wSets
      .filter((s) => s.workoutExerciseId === wex.id && s.isCompleted)
      .sort((a, b) => a.setNumber - b.setNumber)
    if (sets.length) {
      return sets.map((s) => ({ weightKg: s.weightKg, reps: s.reps, rir: s.rir ?? 1 }))
    }
  }
  return null
}
...
interface WorkoutState {
  ...
  updateSet: (activeExerciseId: string, setId: string, patch: Partial<ActiveSet>) => Promise<void>
  ...
  return {
    ...
    loadActive: async () => {
      const [stored, coachStored] = await Promise.all([
        ...
        dbGet<{ id: string; suggestions: Record<string, CoachSuggestion> }>('settings', COACH_KEY),
      ])
      set({
    ...
    startWorkout: async (routineId, dayId) => {
      ...
      for (const p of planned) {
        ...
        const name = library.find((e) => e.id === p.exerciseId)?.name ?? 'Ejercicio'
        const suggestion = suggestions[p.exerciseId]
        const sets: ActiveSet[] = Array.from({ length: p.setsTarget }, (_, index) => {
          const baseWeight = previous?.[index]?.weightKg ?? previous?.[0]?.weightKg ?? 0
          const adjusted =
    ...
    addExercise: async (exerciseId) => {
      ...
      if (!current) return
      const library = useExerciseStore.getState().exercises
      const name = library.find((e) => e.id === exerciseId)?.name ?? 'Ejercicio'
      const next: ActiveSession = {
        ...
        exercises: [
          ...current.exercises,
          {
            ...
            repsTarget: '8-12',
            sets: [{ id: uid(), weightKg: 0, reps: 8, rir: 1, isFailure: false, isCompleted: false, notes: '' }],
          },
        ],
      }
      await commit(next)
    ...
    removeExercise: async (activeExerciseId) => {
      const current = get().active
      if (!current) return
      await commit({ ...current, exercises: current.exercises.filter((e) => e.id !== activeExerciseId) })
    },
    ...
    removeSet: async (activeExerciseId, setId) => {
      ...
      const next: ActiveSession = {
        ...current,
        exercises: current.exercises.map((ex) =>
          ex.id === activeExerciseId ? { ...ex, sets: ex.sets.filter((s) => s.id !== setId) } : ex
        ),
      }
      await commit(next)
    },
    ...
    extendRest: (seconds) => set({ restEndsAt: (get().restEndsAt ?? Date.now()) + seconds * 1000 }),
    ...
    finish: async () => {
      ...
      const durationSeconds = Math.max(1, Math.floor((Date.now() - new Date(current.startedAt).getTime()) / 1000))
      const withSets = current.exercises.filter((ex) => ex.sets.some((s) => s.isCompleted))
      let volume = 0
